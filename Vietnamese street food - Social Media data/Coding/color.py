...
def percentage_data_scene():
    #function which prints the 4 below 
...
def scene_color(AI_visual_file):
    #function which creates top 3 color in 6 lists( name_of_color, percentage_of_color)
    #input the AI_visual_file here to the data_visual varible
    data_visual = [json.loads(line) for line in open(AI_visual_file, 'r')]
    #top 3 color in 6 lists from max_1, max_value_1... to max_value_3
    max1=[]
    max_value1=[]
    max2=[]
    max_value2=[]
    ...
    #the function which go to each scene to take the color and its value
        ...
        if data_scene['end_time (seconds)'][index]+1 < 884:
            for name_of_scene in range(data_scene['Start_time (seconds)'][index],data_scene['end_time (seconds)'][index]+1,1):
                for scene in data_visual[name_of_scene]['imgfeatures']['true_color'].keys():
                    if scene not in new_dict.keys():
                        x=data_visual[name_of_scene]['imgfeatures']['true_color'][scene]
                        new_dict.update({scene:x})
                    else:
                        new_dict[scene]=data_visual[name_of_scene]['imgfeatures']['true_color'][scene]+new_dict[scene]
                sorted_keys = sorted(new_dict, key=new_dict.get)  
                for key in sorted_keys:
                #sort the new dict from smallest to highest
                    sorted_dict[key] = new_dict[key]        
        if sorted_dict != {}:
            #append the value in each annotated scene to the 
...
"""
To extract a color excel file and print the information:
1. Input the annotated scene
2. Run percentage_data_scene() function if you want to see information of the file
3. Run the add_color_value_function with argument AI_visual_file, then run write_excel_file(FileName) with the name of file following with.xlsx 
"""
