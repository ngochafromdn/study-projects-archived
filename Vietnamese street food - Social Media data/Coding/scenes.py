...
def scene_scenes(AI_visual_file):
    #function which creates top 3 color in 6 lists( name_of_scene, percentage_of_scene)
    ...
    #the function which go to each scene to take the color and its value
        ...
        #the dict which contains the color and its total value in each annotated scene
                for scene in data_visual[name_of_scene]['imgfeatures']['scene']['scene'].keys():
                    if scene not in new_dict.keys():
                        x=data_visual[name_of_scene]['imgfeatures']['scene']['scene'][scene]
                        new_dict.update({scene:x})
                    else:
                        new_dict[scene]=data_visual[name_of_scene]['imgfeatures']['scene']['scene'][scene]+new_dict[scene]
...
def add_scene_value_function(AI_visual_file):
#add 6 column lists to the scene_color data frame
    (a,b,c,d,e,f)=scene_scenes(AI_visual_file)
    data_scene['max_1']=a
...
3. Run the add_scene_value_function(AI_visual_file) with argument AI_visual_file, then run write_excel_file(FileName) with the name of file following with.xlsx 
"""
