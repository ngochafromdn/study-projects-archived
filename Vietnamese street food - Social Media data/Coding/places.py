...
def scene_places(AI_visual_file):
    ...
    #the function which go to each scene to take the color and its value
        ...
        for name_of_scene in range(data_scene['Start_time (seconds)'][index],data_scene['end_time (seconds)'][index]+1,1):
                for scene in data_visual[name_of_scene]['imgfeatures']['places']['categories'].keys():
                    if scene not in new_dict.keys():
                        x=data_visual[name_of_scene]['imgfeatures']['places']['categories'][scene]
                        new_dict.update({scene:x})
                    else:
                        new_dict[scene]=data_visual[name_of_scene]['imgfeatures']['places']['categories'][scene]+new_dict[scene]
...
def add_places_value_function(AI_visual_file):
#add 6 column lists to the scene_color data frame
    (a,b,c,d,e,f)=scene_places(AI_visual_file)
    data_scene['max_1']=a
...
3. Run the add_places_value_function with argument AI_visual_file, then run write_excel_file(FileName) with the name of file following with.xlsx 
"""
